...
int main(int argc, char const *argv[]){
    if (argc < 2 || strcmp(argv[argc - 1], "--help") == 0) {
        printf("Usage: %s <socket_id> set/get <DF_Pstate [0-2]>(optional) --debug(optional)\n", argv[0]);
        printf("  <DF_Pstate> : P-state to be imposed to the socket (required for set)\n");
        printf("  --debug     : Enable debug output, prints the actual response from the HSMP message\n");
        printf("Example:  - %s 0 set 2\n", argv[0]);
        printf("          - %s 1 get\n", argv[0]);
        exit(1);
    }
    int file;
    ...
    if (strcmp(argv[2], "set") == 0) {
        ...
        if (argc < 4) {
            printf("Usage: <socket_id> %s set <frequency>\n", argv[0]);
            exit(1);
        }
        // Convert argument to integer
        msg.args[0] = atoi(argv[3]);
    } else if (strcmp(argv[2], "get") == 0) {
        method = GET;
        msg.msg_id = HSMP_GET_FCLK_MCLK;
        msg.num_args = 0;
        msg.response_sz = 2;
    } else if (strcmp(argv[2], "hardcoded") == 0) {
        method = SET;
    ...
    if (method == GET) {
        // Print the response
        printf("Current FCLK: %" PRIu32 " MHz\n", msg.args[0]);
        printf("Current MCLK: %" PRIu32 " MHz\n", msg.args[1]);
    }
